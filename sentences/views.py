...
def sentence_maker(request):
    return Response(OrderedDict((
        ...
        ("/english/<word>/",
         "Fetch English sentences containing a given phrase."),
        ("/espanol/<palabra>/",
         "Busca oraciónes españoles que contiene un frase dada."),
    )))
...
def get_pairs(params):
    ...
    for tmp in soup.find_all('div', class_='translated-sentence-pair'):
        pair = {
            'español': tmp.find('div', class_='spanish-sentence'
                                ).contents[0].strip(),
            'english': tmp.find('div', class_='english-sentence'
                                ).contents[0].strip()
        }
        pairs.append(pair)
    return pairs
